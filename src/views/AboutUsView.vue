<script setup>
import { onMounted, reactive } from 'vue';
import Section from '../components/Section.vue';
import H1 from '../components/H1.vue';
import Tagline from '../components/Tagline.vue';
import H2 from '../components/H2.vue';
import Card from '../components/Card.vue';

const teamMembers = reactive({
    members: [],
    isLoading: true,
    isError: false,
});

onMounted(async () => {
    try {
        const response = await fetch("https://jsonplaceholder.typicode.com/users");

        if (!response.ok) throw new Error("Failed to fetch team data");
        const data = await response.json();
        teamMembers.members = data;

    } catch (error) {
        teamMembers.isError = true;
        console.error(error);
    } finally {
        teamMembers.isLoading = false;
    }
})
</script>

<template>
    <Section>
        <div class="text-center">
            <div class="text-xl font-bold text-black mb-4 -mt-1.75">Our Mission</div>
            <H1>We Empower Developers to Build The Future.</H1>
            <Tagline>In today's digital world, APIs are the backbone of innovation. Yet, managing them has become increasingly complex. API PRO was founded on a simple principle: to give developers and businesses the tools they need to build, manage, and scale their APIs without the headache. We handle the complexity, so you can focus on creating amazing products.</Tagline>
        </div>
        <section class="pt-16 xl:pt-32">
            <H2>The Team Behind API PRO</H2>
            <ul v-if="teamMembers.isLoading" class="md:flex md:flex-wrap md:gap-4">
                <Card v-for="(_, index) in 10" :key="index" class="h-51.25 max-w-80 mx-auto md:max-w-none animate-pulse"></Card>
            </ul>
            <p v-else-if="teamMembers.isError" class="text-center">Failed to load the data. Please try again.</p>
            <ul v-else class="md:flex md:flex-wrap md:gap-4">
                <Card v-for="(member, index) in teamMembers.members" :key="index" custom-class="max-w-80 mx-auto md:max-w-none">
                    <svg width="80px" height="80px" viewBox="0 0 24 24" fill="none" class="bg-neutral-200 rounded-lg mb-4.25">
                        <path d="M14.3365 12.3466L14.0765 11.9195C13.9082 12.022 13.8158 12.2137 13.8405 12.4092C13.8651 12.6046 14.0022 12.7674 14.1907 12.8249L14.3365 12.3466ZM9.6634 12.3466L9.80923 12.8249C9.99769 12.7674 10.1348 12.6046 10.1595 12.4092C10.1841 12.2137 10.0917 12.022 9.92339 11.9195L9.6634 12.3466ZM4.06161 19.002L3.56544 18.9402L4.06161 19.002ZM19.9383 19.002L20.4345 18.9402L19.9383 19.002ZM16 8.5C16 9.94799 15.2309 11.2168 14.0765 11.9195L14.5965 12.7737C16.0365 11.8971 17 10.3113 17 8.5H16ZM12 4.5C14.2091 4.5 16 6.29086 16 8.5H17C17 5.73858 14.7614 3.5 12 3.5V4.5ZM7.99996 8.5C7.99996 6.29086 9.79082 4.5 12 4.5V3.5C9.23854 3.5 6.99996 5.73858 6.99996 8.5H7.99996ZM9.92339 11.9195C8.76904 11.2168 7.99996 9.948 7.99996 8.5H6.99996C6.99996 10.3113 7.96342 11.8971 9.40342 12.7737L9.92339 11.9195ZM9.51758 11.8683C6.36083 12.8309 3.98356 15.5804 3.56544 18.9402L4.55778 19.0637C4.92638 16.1018 7.02381 13.6742 9.80923 12.8249L9.51758 11.8683ZM3.56544 18.9402C3.45493 19.8282 4.19055 20.5 4.99996 20.5V19.5C4.70481 19.5 4.53188 19.2719 4.55778 19.0637L3.56544 18.9402ZM4.99996 20.5H19V19.5H4.99996V20.5ZM19 20.5C19.8094 20.5 20.545 19.8282 20.4345 18.9402L19.4421 19.0637C19.468 19.2719 19.2951 19.5 19 19.5V20.5ZM20.4345 18.9402C20.0164 15.5804 17.6391 12.8309 14.4823 11.8683L14.1907 12.8249C16.9761 13.6742 19.0735 16.1018 19.4421 19.0637L20.4345 18.9402Z" fill="#000000" id="path1" />
                    </svg>
                    <h3 class="text-black font-bold text-lg mb-0.75">{{ member.name }}</h3>
                    <a :href="`mailto:${member.email}`" target="_blank" rel="noopener noreferrer" class="block mb-1">{{ member.email }}</a>
                    <a :href="`https://${member.website}`" target="_blank" rel="noopener noreferrer" class="block -mb-1.75">{{ member.website }}</a>
                </Card>
            </ul>
        </section>
    </Section>
</template>